...
coldwallet-watch 当前已支持添加 EVM 观察钱包并识别链族，但 `HomeScreen` 对 EVM 钱包直接跳过余额查询，显示 "EVM 链余额查询暂不支持"。随着产品需要覆盖 BSC、Arbitrum、Base 等多条 EVM 链，必须在观察端补齐 EVM 资产余额展示能力。
...
6. **链覆盖**
   - 支持 `ChainRegistry` 中已配置的全部 3 条 EVM 链及其测试网：
