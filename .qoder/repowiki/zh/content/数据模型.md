...
- coldwallet-app：离线签名端（冷端），负责生成二维码/文件、解析未签名交易、私钥签名、输出已签名交易。
...
D["models/watch_wallet.dart"]
E["models/asset_balance.dart"]
F["models/cold_export.dart"]
G["models/cold_import.dart"]
end
B --> |"导出未签名交易"| F
G --> |"导入已签名交易"| C
D --> |"只读地址/网络元数据"| A
...
- [coldwallet-watch/lib/models/README.md:1-27](file://coldwallet-watch/lib/models/README.md#L1-L27)

## 核心组件
- WalletInfo：钱包元数据（不含敏感信息），用于本地安全存储的钱包列表。
- ColdExport：未签名交易导出格式，包含版本、类型、网络、交易 CBOR 和摘要。
...
## 架构总览
冷热两端通过“未签名交易”和“已签名交易”两种载体进行交换，载体以 JSON 包装 CBOR 字符串，便于二维码或文件传输。
...
热端->>热端 : "构建未签名交易<br/>生成 ColdExport(JSON+txCbor)"
热端-->>冷端 : "二维码/文件传递 ColdExport"
冷端->>冷端 : "解析 ColdExport, 展示 TxSummary"
冷端->>冷端 : "私钥签名, 生成 signedTx.cbor"
...
- 关键字段
  ...
  - summary：交易摘要，便于用户确认。
- 序列化
  - toJson/fromJson：嵌套对象，summary 包含 assets 列表。
- 约束与验证
...
章节来源
- [coldwallet-app/lib/models/cold_export.dart:41-77](file://coldwallet-app/lib/models/cold_export.dart#L41-L77)
...
章节来源
- [coldwallet-app/lib/models/cold_export.dart:79-108](file://coldwallet-app/lib/models/cold_export.dart#L79-L108)
...
章节来源
- [coldwallet-app/lib/models/cold_export.dart:20-38](file://coldwallet-app/lib/models/cold_export.dart#L20-L38)
...
## 结论
ColdWallet 的数据模型围绕“未签名交易导出”和“已签名交易导入”两条主线设计，结构清晰、职责明确。通过统一的 JSON 包装 CBOR 的方式，实现了冷热端的安全、可靠通信。各模型具备完善的序列化/反序列化能力，便于本地存储与跨端传输。后续可在兼容性、错误提示与大交易传输体验方面持续优化。
...
Build --> Export["导出二维码/文件"]
Export --> Parse["冷端解析 ColdExport"]
Parse --> Confirm["展示 TxSummary 并确认"]
Confirm --> Sign["离线签名生成 signedTx.cbor"]
Sign --> Import["封装为 ColdImport"]
Import --> Submit["热端导入并提交流程"]
Submit --> End(["结束"])
```
