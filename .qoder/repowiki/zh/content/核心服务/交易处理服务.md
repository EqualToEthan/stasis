...
- [coldwallet-app/test/services/cardano_adapter_stake_test.dart](file://coldwallet-app/test/services/cardano_adapter_stake_test.dart)
- [coldwallet-app/test/models/certificate_test.dart](file://coldwallet-app/test/models/certificate_test.dart)
...
## 更新摘要
**变更内容**
- Cardano适配器增强以支持质押交易处理，包括证书注册、委托和提款签名
- 新增Certificate模型支持三种质押证书类型（stakeRegistration、stakeDelegation、stakeDeregistration）
- ColdExport扩展支持certificates、withdrawals和stakeKeyPath字段
- CardanoAdapter实现智能质押交易检测，自动选择普通支付或质押交易签名流程
- 通过TxSigningBundle让SDK自动处理stake key见证集签名
...
- **增强**：Cardano适配器支持完整的质押交易处理，包括证书和提款签名。
...
A3["CardanoAdapter<br/>Cardano签名+质押支持"]
...
- [coldwallet-watch/lib/models/cold_export.dart:1-138](file://coldwallet-watch/lib/models/cold_export.dart#L1-L138)
- [coldwallet-app/lib/services/transaction_service.dart:13-109](file://coldwallet-app/lib/services/transaction_service.dart#L13-L109)
...
- **链适配器层**：通过ChainAdapter接口定义统一的签名抽象，具体实现包括CardanoAdapter（CIP-1852路径+CBOR交易+Ed25519签名+质押交易支持）和EvmAdapter（BIP-44路径+RLP交易+EIP-155/EIP-1559签名）。
- **链注册中心**：ChainRegistry管理所有支持的链配置，提供适配器实例查找功能。
- 数据模型：ColdExport（未签名交易，支持质押字段）、ColdImport（已签名交易）、SignResult（统一签名结果）、ChainConfig（链配置）、TxSummary（交易摘要）、AssetAmount（资产项）、Certificate（质押证书）。
...
## 架构总览
下图展示从观察端构建未签名交易到冷钱包签名并返回的端到端流程，包括QR码传输与JSON编解码，以及新的多链适配机制和质押交易处理。
...
Note over Watch : 质押交易包含certificates/withdrawals/stakeKeyPath
Watch-->>Export : 生成ColdExport(txCbor, summary, staking fields)
...
- [coldwallet-watch/lib/services/tx_builder_service.dart:11-118](file://coldwallet-watch/lib/services/tx_builder_service.dart#L11-L118)
- [coldwallet-watch/lib/models/cold_export.dart:1-138](file://coldwallet-watch/lib/models/cold_export.dart#L1-L138)
- [coldwallet-app/lib/services/transaction_service.dart:76-109](file://coldwallet-app/lib/services/transaction_service.dart#L76-L109)
...
- **CardanoAdapter**：封装现有Cardano签名逻辑，使用CIP-1852路径派生地址，CBOR交易解析，Ed25519签名，**增强支持质押交易**。
...
+signTransaction(mnemonic, coldExport, config) SignResult
-_signStakingTransaction(wallet, tx, export) WitnessSet
}
class EvmAdapter {
+chainFamily "evm"
+deriveAddress(mnemonic, config) String
...
**图表来源**
- [coldwallet-app/lib/services/adapters/chain_adapter.dart:1-37](file://coldwallet-app/lib/services/adapters/chain_adapter.dart#L1-L37)
- [coldwallet-app/lib/services/adapters/cardano_adapter.dart:1-147](file://coldwallet-app/lib/services/adapters/cardano_adapter.dart#L1-L147)
...
### Cardano适配器质押交易处理
**新增**：Cardano适配器现已完全支持质押交易处理，包括证书注册、委托、注销和奖励提款。

- **智能交易检测**：自动检测export.certificates或export.withdrawals是否存在，决定使用普通支付还是质押交易签名流程。
- **质押交易签名**：通过TxSigningBundle让SDK自动处理stake key见证集签名，支持stake registration、delegation、deregistration和reward withdrawals。
- **证书类型支持**：支持三种证书类型（stakeRegistration、stakeDelegation、stakeDeregistration），每种类型都有特定的参数要求。
- **提款处理**：支持从奖励地址提取LOVELACE，自动验证stake credential匹配。

```mermaid
flowchart TD
Start(["CardanoAdapter.signTransaction"]) --> CheckFields{"检查certificates/withdrawals"}
CheckFields -- 无质押字段 --> PaymentTx["普通支付交易"]
CheckFields -- 有质押字段 --> StakingTx["质押交易"]
PaymentTx --> PaymentSign["wallet.signTransaction<br/>仅payment key签名"]
StakingTx --> Bundle["构建TxSigningBundle"]
Bundle --> StakeSign["wallet.signTransactionsBundle<br/>自动添加stake key witness"]
PaymentSign --> Combine["合并见证集"]
StakeSign --> Combine
Combine --> Hash["计算交易哈希"]
Hash --> Result["返回SignResult"]
```

**图表来源**
- [coldwallet-app/lib/services/adapters/cardano_adapter.dart:41-75](file://coldwallet-app/lib/services/adapters/cardano_adapter.dart#L41-L75)
- [coldwallet-app/lib/services/adapters/cardano_adapter.dart:77-134](file://coldwallet-app/lib/services/adapters/cardano_adapter.dart#L77-L134)

**章节来源**
- [coldwallet-app/lib/services/adapters/cardano_adapter.dart:41-134](file://coldwallet-app/lib/services/adapters/cardano_adapter.dart#L41-L134)
- [coldwallet-app/test/services/cardano_adapter_stake_test.dart:1-110](file://coldwallet-app/test/services/cardano_adapter_stake_test.dart#L1-L110)
...
- 流程：
  ...
  - **质押交易扩展**：支持添加certificates、withdrawals和stakeKeyPath字段。
...
Serialize --> AddStaking{"是否质押交易?"}
AddStaking -- 否 --> ReturnExport["返回ColdExport(txCbor, summary)"]
AddStaking -- 是 --> AddFields["添加certificates/withdrawals/stakeKeyPath"]
AddFields --> ReturnExport
```

**图表来源**
- [coldwallet-watch/lib/services/tx_builder_service.dart:11-118](file://coldwallet-watch/lib/services/tx_builder_service.dart#L11-L118)
- [coldwallet-watch/lib/models/cold_export.dart:15-22](file://coldwallet-watch/lib/models/cold_export.dart#L15-L22)

**章节来源**
- [coldwallet-watch/lib/services/tx_builder_service.dart:11-118](file://coldwallet-watch/lib/services/tx_builder_service.dart#L11-L118)

### 数据模型与 CBOR 编解码
- **ColdExport**：包含version、type、network、txCbor、summary。**增强支持**：certificates（质押证书列表）、withdrawals（奖励提款映射）、stakeKeyPath（stake key派生路径）。
...
- **Certificate**：**新增**质押证书模型，支持三种类型：stakeRegistration、stakeDelegation、stakeDeregistration。
...
- **质押交易支持**：certificates数组包含证书对象，withdrawals映射包含奖励地址到金额的对应关系。
...
- **质押交易支持**：对于质押交易，SDK自动处理stake key见证集签名，无需手动指定stake key路径。
...
- 冷钱包应用：
  ...
  - **质押交易支持**：证书类型未知时抛出StateError，stake key不匹配时抛出异常。
- 数据校验：
...
- [coldwallet-app/lib/models/certificate.dart:35-46](file://coldwallet-app/lib/models/certificate.dart#L35-L46)
...
- 容量预估：
  ...
  - **质押交易扩展**：包含certificates和withdrawals字段的JSON略大，但仍可在单张QR码内承载。
...
- **质押交易支持**：CardanoAdapter依赖cardano_flutter_sdk的TxSigningBundle进行质押交易签名。
...
I --> J["CardanoAdapter"]
I --> K["EvmAdapter"]
J --> L["cardano_flutter_sdk"]
C -- "JSON/CBOR" --> D
M["ColdImport"] -- "JSON/CBOR" --> A
N["SignResult"] -- "统一结果" --> D
O["ChainConfig"] -- "链配置" --> H
P["Certificate"] -- "质押证书" --> C
```
...
- [coldwallet-app/lib/services/transaction_service.dart:1-109](file://coldwallet-app/lib/services/transaction_service.dart#L1-L109)
- [coldwallet-app/lib/services/chain_registry.dart:1-79](file://coldwallet-app/lib/services/chain_registry.dart#L1-L79)
- [coldwallet-app/lib/services/adapters/chain_adapter.dart:1-37](file://coldwallet-app/lib/services/adapters/chain_adapter.dart#L1-L37)
- [coldwallet-app/lib/services/adapters/cardano_adapter.dart:1-147](file://coldwallet-app/lib/services/adapters/cardano_adapter.dart#L1-L147)
...
- **质押交易优化**：TxSigningBundle让SDK自动处理复杂的stake key见证集逻辑，减少手动处理开销。
...
- 冷钱包应用常见问题：
  ...
  - **质押交易支持**："Unknown certificate type"：检查证书类型是否正确。
  - **质押交易支持**："Stake key mismatch"：检查stake credential是否与预期匹配。
- 数据解析问题：
...
## 结论
本交易处理服务通过ColdExport/ColdImport协议实现了观察端与冷钱包的安全分离，**现已升级为多链支持并完全支持Cardano质押交易**。TransactionService提供了统一的签名接口（signForChain()），能够自动识别链类型并路由到相应的链适配器，支持Cardano、EVM等多种区块链。**重大增强**：Cardano适配器现已完全支持质押交易处理，包括证书注册、委托、注销和奖励提款的完整签名流程，通过TxSigningBundle让SDK自动处理复杂的stake key见证集逻辑。通过ChainAdapter接口设计，系统具有良好的扩展性，可以轻松添加新的链族支持。原有的Cardano签名流程保持不变，确保向后兼容性。结合CIP-1852路径的密钥派生和多链适配器架构，确保了交易的安全性与合规性。QR码传输方案简洁可靠，适合MVP阶段的快速交付。后续可扩展更多链族支持、智能合约交互与更复杂的费用估算策略。

## 附录
- 关键代码片段路径（不包含具体代码内容）：
  - 统一签名入口：[coldwallet-app/lib/services/transaction_service.dart:76-109](file://coldwallet-app/lib/services/transaction_service.dart#L76-L109)
  ...
  - Cardano适配器实现：[coldwallet-app/lib/services/adapters/cardano_adapter.dart:1-147](file://coldwallet-app/lib/services/adapters/cardano_adapter.dart#L1-L147)
  - 质押交易签名：[coldwallet-app/lib/services/adapters/cardano_adapter.dart:77-134](file://coldwallet-app/lib/services/adapters/cardano_adapter.dart#L77-L134)
  ...
  - 证书模型：[coldwallet-app/lib/models/certificate.dart:1-48](file://coldwallet-app/lib/models/certificate.dart#L1-L48)
  ...
  - Cardano质押交易测试：[coldwallet-app/test/services/cardano_adapter_stake_test.dart:1-110](file://coldwallet-app/test/services/cardano_adapter_stake_test.dart#L1-L110)
  - 证书测试：[coldwallet-app/test/models/certificate_test.dart:1-56](file://coldwallet-app/test/models/certificate_test.dart#L1-L56)