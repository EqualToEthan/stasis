...
- **新增Cardano stake地址显示功能**：当选择Cardano链时自动派生并显示stake地址，提供复制功能和合并二维码生成
- **增强质押交易支持**：在Cardano适配器中实现完整的质押交易签名逻辑，支持注册、委托和解除注册操作
...
- **Cardano stake地址显示**：当选择Cardano链时自动派生并显示stake地址，提供复制功能和合并二维码生成。
- 网络切换：在主网与测试网之间切换，并持久化到安全存储。
- **链联动扫码签名入口**：跳转到扫码页面，扫描未签名交易二维码后进入交易详情，自动校验链匹配。
- **链联动文件导入入口**：从文件或粘贴 JSON 导入未签名交易，解析后进入交易详情，自动校验链匹配。
- **质押交易支持**：完整支持Cardano质押交易的签名，包括注册、委托和解除注册操作。
- 钱包管理入口：跳转至钱包管理页，支持查看地址、备份助记词、删除钱包等。

同时说明状态管理机制（加载钱包列表、当前钱包、网络设置、多链地址缓存）、用户交互流程、错误处理策略以及与服务层的集成方式。**通过 ChainRegistry 和 ChainAdapter 模式实现跨区块链网络的统一签名访问，新增的多链地址管理和Cardano stake地址功能提供了直观的下拉界面和智能缓存机制。**
...
- 模型层：wallet_info.dart、cold_export.dart、cold_import.dart、certificate.dart 描述数据契约。
...
- [coldwallet-app/lib/services/wallet_service.dart:13-246](file://coldwallet-app/lib/services/wallet_service.dart#L13-L246)
...
## 核心组件
- 首页 HomeScreen：承载钱包选择器、**多链地址下拉选择器**、**Cardano stake地址显示**、网络切换按钮、扫码签名、文件导入、钱包管理等入口；维护本地状态（钱包列表、当前钱包、网络、加载态、多链地址缓存）。
- 钱包服务 WalletService：提供助记词生成/校验、HD 钱包创建、**多链地址派生**、**stake地址派生**、网络设置、钱包增删改查、PIN 管理等。
- **链注册中心 ChainRegistry**：管理所有支持的链配置，提供适配器实例查找和链配置查询。
- **链适配器 ChainAdapter**：抽象接口定义地址派生、交易解析和签名的链特有逻辑。
- **Cardano适配器**：实现 Cardano 链的地址派生（CIP-1852）、交易解析（CBOR）和离线签名（Ed25519），**支持完整的质押交易签名**。
...
- 模型 WalletInfo/ColdExport/ColdImport/SignResult/Certificate：描述钱包元数据、未签名交易导出格式、已签名交易导入格式、统一签名结果、**质押证书**。

**章节来源**
- [coldwallet-app/lib/screens/home_screen.dart:16-619](file://coldwallet-app/lib/screens/home_screen.dart#L16-L619)
- [coldwallet-app/lib/services/wallet_service.dart:13-246](file://coldwallet-app/lib/services/wallet_service.dart#L13-L246)
- [coldwallet-app/lib/services/chain_registry.dart:1-79](file://coldwallet-app/lib/services/chain_registry.dart#L1-L79)
...
## 架构总览
首页通过 WalletService 与 SecureStorageService 协作，完成钱包列表与网络设置的读取与更新；**通过 ChainRegistry 和 ChainAdapter 模式实现多链地址派生和统一签名访问**；通过路由导航到扫码或钱包管理页；在交易导入流程中，将 ColdExport 传递给交易详情页，后续由相应的链适配器完成签名并产出 SignResult。**新增的Cardano stake地址功能在首页加载时自动检测链类型并派生相应地址。**
...
H->>H : 检查是否为Cardano链
alt Cardano链选中
H->>WS : deriveStakeAddress(mnemonic)
WS-->>H : 返回stake地址
H->>H : 显示stake地址和复制功能
end
H->>H : 渲染多链地址下拉选择器
...
- [coldwallet-app/lib/services/wallet_service.dart:95-116](file://coldwallet-app/lib/services/wallet_service.dart#L95-L116)
...
- **Cardano stake地址显示**：当检测到选中的链为Cardano时，自动派生并显示stake地址，提供复制功能和合并二维码生成。
...
状态管理
- 本地字段：`_wallets`、`_currentWallet`、`_isLoading`、`_allAddresses`（多链地址缓存）、`_addressesLoading`（地址加载状态）、`_selectedChainId`（选中链ID）、**`_stakeAddress`（Cardano stake地址）**。
...
- **stake地址加载**：`_loadStakeAddressIfNeeded` -> 检测Cardano链 -> 派生stake地址 -> 更新UI。
...
MultiChain --> CheckCardano{"是否选中Cardano链?"}
CheckCardano -- "是" --> LoadStake["派生stake地址"]
CheckCardano -- "否" --> RenderDropdown["渲染下拉选择器"]
LoadStake --> RenderDropdown
RenderDropdown --> Action{"用户操作"}
Action --> |切换钱包| Switch["切换当前钱包并刷新"]
Action --> |复制地址| Copy["复制到剪贴板"]
Action --> |复制stake| CopyStake["复制stake地址"]
Action --> |显示QR| ShowQR["生成合并二维码"]
...
- [coldwallet-app/lib/screens/home_screen.dart:520-548](file://coldwallet-app/lib/screens/home_screen.dart#L520-L548)
...
- **stake地址派生**：使用 CIP-1852 路径 m/1852'/1815'/0'/2/0 派生stake地址。
...
- **stake地址支持**：deriveStakeAddress 方法专门用于Cardano链的stake地址派生。
...
- **质押交易签名**：构建 TxSigningBundle 让 SDK 自动添加 stake key witness，支持注册、委托和解除注册操作
...
### 模型 WalletInfo/ColdExport/ColdImport/SignResult/Certificate
- WalletInfo：钱包元数据（id、name、createdAt），提供 JSON 编解码与唯一 ID 生成。
- ColdExport：插件端导出的未签名交易数据（version、type、network、txCbor、summary），**质押交易额外包含 certificates、withdrawals、stakeKeyPath**。
- ColdImport：冷钱包签名后的导出数据（version、type、txCbor、txHash）。
- **SignResult**：统一的签名结果，包含已签名交易的 hex 编码和交易哈希，支持版本控制。
- **Certificate**：Cardano 质押证书类型，支持 stakeRegistration、stakeDelegation、stakeDeregistration 三种操作。

**章节来源**
- [coldwallet-app/lib/models/wallet_info.dart:8-56](file://coldwallet-app/lib/models/wallet_info.dart#L8-L56)
- [coldwallet-app/lib/models/cold_export.dart:5-137](file://coldwallet-app/lib/models/cold_export.dart#L5-L137)
...
- **stake地址懒加载**：仅在Cardano链选中时派生stake地址，避免不必要的计算开销。
- **合并二维码优化**：Cardano链的合并二维码仅在需要时生成，减少内存占用。
...
- **stake地址派生失败**：检查Cardano链配置和网络设置，确认助记词有效性。
- **质押交易签名失败**：检查证书格式和stake key路径，确认质押操作类型。
...
- **在stake地址派生过程中记录网络设置和派生结果**。
- **在质押交易签名过程中记录证书信息和stake key路径**。
...
## 结论
首页作为冷钱包的核心入口，提供了直观的钱包选择、**多链地址下拉选择器**、**Cardano stake地址显示**、网络切换、扫码签名、文件导入与钱包管理入口。**通过 ChainRegistry 和 ChainAdapter 模式实现了跨区块链网络的统一签名访问，支持自动链类型检测和路由**。新增的多链地址管理功能通过下拉界面和智能缓存机制，显著提升了用户体验和性能。**新增的Cardano stake地址功能为用户提供了完整的质押支持，包括地址显示、复制功能和合并二维码生成**。通过 WalletService 与 SecureStorageService 的配合，实现了安全的状态管理与持久化。结合链适配器系统，完成了从导入未签名交易到离线签名的完整闭环，支持 Cardano 和多种 EVM 兼容链。整体设计清晰、职责分明，便于扩展和维护新的区块链网络。
...
- **Cardano stake地址支持**：使用 CIP-1852 标准派生stake地址，支持testnet和mainnet。
- **质押交易完整支持**：实现注册、委托、解除注册三种质押操作的签名逻辑。
- **合并二维码功能**：为观察钱包提供payment address和stake address的合并二维码。
...
- [coldwallet-app/lib/services/chain_registry.dart:11-55](file://coldwallet-app/lib/services/chain_registry.dart#L11-L55)
...
- [coldwallet-app/lib/screens/home_screen.dart:520-617](file://coldwallet-app/lib/screens/home_screen.dart#L520-L617)