...
| 文件 | 主要类 | 功能说明 |
|------|--------|----------|
| confirm_sign_screen.dart | ConfirmSignScreen | PIN 验证并签名交易，支持 Cardano 和 EVM 多链签名路由；对 WalletMismatchException 直接显示其消息，避免普通“签名失败”前缀 |
| dice_entropy_screen.dart | DiceEntropyScreen | 骰子熵收集，掷物理骰子 256 次收集真随机熵生成 BIP-39 助记词 |
| export_signed_screen.dart | ExportSignedScreen | 已签名交易导出，展示交易哈希、二维码，支持复制完整签名数据 |
| home_screen.dart | HomeScreen | 首页，钱包选择器和操作入口（扫码签名、粘贴导入、钱包管理）。扫码/导入时从 JSON 自动检测链类型，无需显式链选择器 |
| scan_tx_screen.dart | ScanTxScreen | 扫描交易二维码，从 JSON 自动检测链类型并校验是否为已注册链，通过后跳转交易详情页 |
| tx_detail_screen.dart | TxDetailScreen | 交易详情，根据链类型展示 Cardano 或 EVM 交易摘要；Cardano 质押交易额外显示押金信息（正数=注册押金，负数=退回押金）、奖励提取金额和 DRep 委托卡片（含 voteDelegation 证书时，按 dRepType 展示弃权/不信任/截断 hash） |
...
- **外部依赖**：mobile_scanner、qr_flutter、flutter/services（Clipboard、HapticFeedback）
...
HomeScreen（粘贴导入）→ TxDetailScreen → ConfirmSignScreen → ExportSignedScreen
```
...
| 修改地址不匹配时的提示文案 | confirm_sign_screen.dart — 修改 _verifyAndSign 中对 WalletMismatchException 的处理，或 transaction_service.dart 中 WalletMismatchException 的 message |
