...
冷钱包的核心业务逻辑层。封装了安全存储、钱包密钥管理和交易签名等关键功能。完全离线运行，所有敏感数据通过 Android Keystore 加密存储。
...
| transaction_service.dart | TransactionService | 交易签名，解析未签名交易 CBOR，用本地私钥签名并生成已签名交易 |
| wallet_service.dart | WalletService | 钱包服务，助记词生成/验证、HD 钱包创建、地址派生、多钱包管理和 PIN 管理 |
...
| `deriveAddress(mnemonic, {testnet})` | `Future<String>` | 派生 CIP-1852 地址（m/1852'/1815'/0'/0/0） |
...
| 方法 | 返回值 | 说明 |
|------|--------|------|
| `parseColdExport(jsonString)` | `ColdExport` | 从 JSON 字符串解析未签名交易数据 |
| `signTransaction(coldExport)` | `Future<ColdImport>` | 用当前钱包私钥签名交易，返回已签名数据 |
...
- **内部依赖**：
  - `models/` — WalletInfo、ColdExport、ColdImport
- **外部依赖**：
